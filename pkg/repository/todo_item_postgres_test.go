...
func TestTodoItemPostgres_Create(t *testing.T) {
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			testCase.mockBehavior(testCase.args, testCase.id) //Задаем поведение для функции мока
...
func TestTodoItemPostgres_GetAll(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func() {
				...
				mock.ExpectQuery("SELECT (.+) FROM todo_items ti INNER JOIN list_items li on (.+) INNER JOIN users_lists ul on (.+) WHERE (.+)").
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.mock() //Задаем поведение функциии мока
...
func TestTodoItemPostgres_Delete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "OK",
			mock: func() {
				mock.ExpectExec("DELETE FROM todo_items ti USING list_items li, users_lists ul WHERE (.+)").
...
func TestTodoItemPostgres_Update(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "OK_AllFields",
			mock: func() {
				mock.ExpectExec("UPDATE todo_items ti SET (.+) FROM list_items li, users_lists ul WHERE (.+)").
