...
func TestTodoListPostgres_Delete(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mock: func() {
				mock.ExpectExec("DELETE FROM todo_lists tl USING users_lists ul WHERE (.+)").
					WithArgs(401, 1).WillReturnError(sql.ErrNoRows)
			},
			input: args{
				listId: 401,
				userId: 1,
...
func TestTodoListPostgres_Update(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func() {
				mock.ExpectExec("UPDATE todo_lists tl SET (.+) FROM users_lists ul WHERE (.+)").
					WithArgs("new_title", "new_description", 1, 1).WillReturnResult(sqlmock.NewResult(0, 1))
			},
			input: args{
				listId: 1,
				userId: 1,
		...
		},
		{
			...
			mock: func() {
				mock.ExpectExec("UPDATE todo_lists tl SET (.+) FROM users_lists ul WHERE (.+)").
					WithArgs("new_title", 1, 1).WillReturnResult(sqlmock.NewResult(0, 1))
			},
			input: args{
				listId: 1,
				userId: 1,
		...
		},
		{
			...
			mock: func() {
				mock.ExpectExec("UPDATE todo_lists tl SET (.+) FROM users_lists ul WHERE (.+)").
					WithArgs("new_description", 1, 1).WillReturnResult(sqlmock.NewResult(0, 1))
			},
			input: args{
				listId: 1,
				userId: 1,
